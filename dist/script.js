...
window.addEventListener('DOMContentLoaded', function () {
  ...
  modulesSlider.init();
  var feSlider = new _modules_slider_slider_mini__WEBPACK_IMPORTED_MODULE_1__["default"]({
    ...
    value: function bindTriggers(container, items, counter) {
      container.querySelector('.plus').addEventListener('click', function () {
        if (counter != items.length - 2) {
          ...
          // Дошли до последней карточки, показываем ее и удаляем плюс-карточку
          items[counter].style.display = 'flex';
          items[items.length - 1].remove();
        }
      });
    } // скрываем все кром последнего блока
    ...
    value: function init() {
      ...
      this.forms.forEach(function (item) {
        item.addEventListener('submit', function (e) {
          ...
          statusMess.textContent = _this.message.loading;
          var formData = new FormData(item);

          _this.postData(_this.path, formData).then(function (res) {
            console.log(res);
            statusMess.textContent = _this.message.success;
          }).catch(function () {
            statusMess.textContent = _this.message.failure;
          }).finally(function () {
            _this.clearInputs();
...
function () {
  function VideoPlayer(triggers, overlay) {
    ...
    this.close = this.overlay.querySelector('.close'); // привязываем это свойство жестко к объекту
  ...
  _createClass(VideoPlayer, [{
    ...
    value: function bindTriggers() {
      ...
      this.btns.forEach(function (btn, i) {
        ...
        btn.addEventListener('click', function () {
          if (!btn.closest('.mobule_video-item') || btn.closest('.module__video-item').getAttribute('data-disabled') !== 'true') {
            _this.activeBtn = btn; // проверяем создали ли уже айфрейм
    ...
    value: function createPlayer(url) {
      this.player = new YT.Player('frame', {
        ...
        videoId: "".concat(url),
        events: {
          // чтобы работало привязали контекст в конструкторе
...
function (_Slider) {
  ...
  function MainSlider(btns) {
    ...
    // какие свойства наследуются от прототипа
...
var Slider = // пустые строки и объект для избежания ошибок
function Slider() {
